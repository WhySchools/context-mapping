...
_SKIP_DIRS = {"node_modules", "dist", ".git", "target", ".vite"}
...
def _parse_ts_function(node: Node, src: bytes) -> Optional[FunctionInfo]:
    ...
    return FunctionInfo(
        name=name, is_public=True, is_async=is_async,
        params=params, return_type=ret_type, doc_comment=doc,
...
def _parse_ts_interface(node: Node, src: bytes) -> Optional[StructInfo]:
    ...
    doc = _extract_jsdoc(node, src)
    return StructInfo(name=name, is_public=True, fields=fields, doc_comment=doc, derives=[])
...
def parse_ts_directory(dir_path: Path, project_root: Path) -> ModuleContext:
    ...
    ts_files = sorted(list(dir_path.glob("*.ts")) + list(dir_path.glob("*.tsx")))
    for f in ts_files:
...
def _find_ts_dirs(root: Path) -> list[Path]:
    ...
    for ts in list(root.rglob("*.ts")) + list(root.rglob("*.tsx")):
        if any(p in ts.parts for p in _SKIP_DIRS):
            continue
